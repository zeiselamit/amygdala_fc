...
gad2 = data(strcmpi(geneid,'Gad2'),:);

% validcells = (tot_mol>4000 & tot_mol<5e4 & tot_genes>2000 & sum(data(loc,:)>0)<=2 & (stmn2>0 | snap25>0) & amy_flag');
validcells = (tot_mol>2000 & tot_mol<5e4 & tot_genes>1000 & amy_flag' & snap25<4 & stmn2<4 & gad2==0 & slc17a7==0 & slc17a6==0); % all cells
...
save(['afterloading_noneuronal_QC2000_FC_',date],'data','geneid','cellid','sample','fc_time','batch_flag','amy_flag','piri_flag','-v7.3');
...
data = round(data./repmat(sum(data),length(data(:,1)),1)*10e3);
...
%     savefig(gcf,['tsne_FC_step1_nonneurons_by_annot_perplexity_',num2str(perplexity),'_PCAdim',num2str(initial_dims),'_',date,'.fig'])
% end
% % % % % 
...
immmark = {'C1qc','C1qa','C1qb','Mrc1','Pf4','Cx3cr1'};
astromark = {'Gja1','Aqp4','Foxj1','Aldoc','Mfge8','Slc25a18'};
vascmark = {'Vtn','Cldn5','Pdgfrb','Flt1','Acta2','Ly6c1'};
oligmark = {'Plxnb3','Cldn11','Olig1','Olig2','Sox10','Pdgfra'};
bloodmark = {'Hbb-bs','Hbb-bt','Hba-a2','Ttr'};

[~,loc] = ismember(immmark,geneid_all);
immmark = sum(data_orig_all_sorted(loc,:));
[~,loc] = ismember(astromark,geneid_all);
astromark = sum(data_orig_all_sorted(loc,:));
[~,loc] = ismember(vascmark,geneid_all);
vascmark = sum(data_orig_all_sorted(loc,:));
[~,loc] = ismember(oligmark,geneid_all);
oligmark = sum(data_orig_all_sorted(loc,:));
[~,loc] = ismember(bloodmark,geneid_all);
bloodmark = sum(data_orig_all_sorted(loc,:));
...
for jjj=1:length(T_cells_tmp_uni)
    ...
    tmp = [mean(immmark(:,T_cells_tmp==jjj));mean(astromark(:,T_cells_tmp==jjj));...
        mean(vascmark(:,T_cells_tmp==jjj));mean(oligmark(:,T_cells_tmp==jjj));mean(bloodmark(:,T_cells_tmp==jjj));....        
        mean(gad2(:,T_cells_tmp==jjj));mean(slc17a7(:,T_cells_tmp==jjj));....
        mean(slc17a6(:,T_cells_tmp==jjj));];
    tmpsort = tmp;%[tmp(1),max(tmp(2:3)),tmp(4)];
...
gabaglutuni = unique(gabaglut);
for idx=1:length(gabaglutuni)
    ii=find(gabaglut_sc==gabaglutuni(idx)); 
...
axis off
title('classes')
leg = {'immune','astro','vascular','oligo','blood','GABA','Glut1','Glut2','doublets'};
legend(leg(gabaglutuni))
...
table1 = [cellid_sorted,m2c(gabaglut_sc)];% order is 'immune','astro','vascular','oligo','blood','GABA','Glut1','Glut2','doublets'
saveCellFile(table1,['cellid_nonneurons_step1_FC_QC2000_',date,'.txt']);
% % % % % % % % % % % % % %
list = {'Hbb-bt','Hba-a2','Ttr','Acta2','Ly6c1','Aldoc','Mfge8','Pf4','Cx3cr1','Foxj1','Pdgfra','Mog','C1qc','Flt1','Cldn5','Aqp4','Plp1'};
figure;
