...
% % % % % % % % % % % % % % % % % % % 
% load clustercolor_GABA_hannah.mat%clustername_color_hannah
tmp = loadCellFile_turbo('GABA-hexa.txt',1);
clustername_color_hannah = tmp(2:end,2:3);
oldname_newname = loadCellFile('new-GABA-names.txt');
[~,loc] = ismember(clusteruni,oldname_newname(:,2));
clusteruni_newnames = oldname_newname(loc,3);
[~,loc] = ismember(clusteruni,clustername_color_hannah(:,1));
% clustername_color_hannah
% colors = distinguishable_colors(length(T_cells_tmp_uni)+1);
figure;
set(gcf,'color','w','position',[20,20,900,800])
for i=1:length(T_cells_tmp_uni)
    ii=find(T_cells_tmp==i); h=plot(mapped_xy(ii,1),mapped_xy(ii,2),'.','color',['#',clustername_color_hannah{loc(i),2}],'markersize',3); hold on;
end
axis tight;
xl = get(gca,'xlim');
yl = get(gca,'ylim');
...
for i=1:length(T_cells_tmp_uni)
    if i>=0
        ...
        ht = text(median(mapped_xy(in,1))+xo,median(mapped_xy(in,2))+yo,regexprep(clusteruni_newnames{i},'_','-'));
...
if savefig_flag==1
    savefig(gcf,['tsne_final_step3_FC_GABA_by_cluster_',date,'.fig'])    
end
if savefig_pdf==1
    eval(['export_fig tsne_final_step3_FC_GABA_by_cluster_',date,'.pdf']);
end
% 
...
h2 = axes('position',[0.38,0.03,0.56,0.93]);
% x=squareform(Dpca); 
x = corr_mat(prj');
imagesc(x(leaforder_pca,leaforder_pca),[prctile(x(:),1),prctile(x(:),99)]);
cm = [[linspace(0,1,500)',linspace(0,1,500)',linspace(1,1,500)'];[linspace(1,1,500)',linspace(1,0,500)',linspace(1,0,500)']];
% cm = [linspace(1,0.4,500)',linspace(1,0.4,500)',linspace(1,0.4,500)'];
colormap((cm))
...
linkaxes([h1,h2],'y');
if savefig_flag==1
    savefig(gcf,['tree_final_step3_FC_GABA_',date,'.fig'])    
end
if savefig_pdf==1
    eval(['export_fig tree_final_step3_FC_GABA_',date,'.pdf']);
end
...
% % % % % % % % % % % % % % 
%%
gabaspatial = loadCellFile('GABA_spatialclusters.txt');
T_cells_tmp_spatial = zeros(length(T_cells_tmp),1);
for i=1:length(T_cells_tmp_uni)
    ii=find(T_cells_tmp(:,1)==i);
    T_cells_tmp_spatial(ii) = gabaspatial{i,4};
end
...
axis off
title(['#C=',num2str(max(T_cells_tmp)),', perplexity=',num2str(perplexity)],'fontsize',8);
[~,ia] = unique(cell2mat(gabaspatial(:,4)));
legend(gabaspatial(ia,5))
if savefig_pdf==1
    eval(['export_fig tsne_final_step3_FC_GABA_by_spatialcluster_',date,'.pdf']);
end
%%
% % % % % % % % % % % % % % 
...
saveCellFile(table1,['FC_final_step3_GABA_dendrogram_junction_split_markers_',date,'.txt']);
saveCellFile(table2,['FC_final_step3_GABA_dendrogram_junction_split_markers_by_average_',date,'.txt']);
...
% plot by tissue
if savefig_flag==1
    eval(['export_fig tsne_final_step3_FC_GABA_by_annot_','_',date,'.pdf']);
    savefig(gcf,['tsne_final_step3_FC_GABA_by_annot_perplexity_',date,'.fig'])
end
...
if savefig_flag==1
    eval(['export_fig tsne_final_step3_FC_GABA_bytimepoint_',date,'.pdf']);
    savefig(gcf,['tsne_final_step3_FC_GABA_bytimepoint_',date,'.fig'])
end
% % 
figure;
set(gcf,'color','w','position',[20,20,1100,960])
...
title('recall')
% if savefig_flag==1
%     eval(['export_fig tsne_final_FC_GABA_bytimepoint_','_',date,'.pdf']);
%     savefig(gcf,['tsne_final_FC_GABA_bytimepoint_',num2str(perplexity),'_PCAdim',num2str(initial_dims),'_',date,'.fig'])
% end
% % % % % % % % % % % % % % % % % % 
...
if savefig_flag==1
    savefig(gcf,['markertable_step3_final_FC_GABA_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
if savefig_pdf==1
    eval(['export_fig markertable_step3_final_FC_GABA_',date,'.pdf']);
end

table1 = [cellid_sorted,m2c(T_cells_tmp)];
saveCellFile(table1,['cellidCluster_step3_GABA_FC_',date,'.txt']);
saveCellFile([m2c([1:56]'),clusteruni(leaforder_pca)],['ClusterOrder_step3_GABA_FC_',date,'.txt']);
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
linkaxes([ax1,ax2_1,ax2_2,ax2_3,ax2_4,ax2_5],'x');
if savefig_flag==1
    savefig(gcf,['junctionmarkers_final_step3_FC_GABA_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
% % % % % % % % % 
...
if savefig_flag==1
    savefig(gcf,['junctionmarkers_final_step3_clusteraverage_FC_GABA_',date,'.fig'])
    eval(['export_fig junctionmarkers_final_step3_clusteraverage_FC_GABA_',date,'.pdf']);
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % % 
% % % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
linkaxes([ax1,h2],'x')
if savefig_flag==1
    savefig(gcf,['CEA_markers_step3_clusteraverage_FC_GABA_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
linkaxes([ax1,h2],'x')
if savefig_flag==1
    savefig(gcf,['N_Z_list_step3_clusteraverage_FC_GABA_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % 
%%
list = loadCellFile('hannah_GABA_list_modular_genes.txt');
[~,ind_list] = ismember(list,geneid_all);
list(ind_list==0) = [];
ind_list(ind_list==0) = [];
datamarkers = meangr_mat_0(ind_list,:);
...
linkaxes([ax1,h2],'x')
if savefig_flag==1
%     savefig(gcf,['hannah_GABA_list_modular_genes_',date,'.fig'])
    eval(['export_fig hannah_GABA_list_modular_genes_',date,'.pdf']);
end
%%
% % % % % % % % % % % % % % % % % % % % % % % % % 
...
for jjj=1:length(list)
    ...
    for k=1:length(T_cells_tmp_uni)
        ...
        if length(y)>10
            ...
            plot(k+0.7*rand(length(gr2),1)-0.1,0.5*rand(length(gr2),1)-0.1+data_orig_all_sorted(g,gr2)','.'); hold on;
        end
...
if savefig_flag==1
    ...
    eval(['export_fig clustermarkers_violin_final_step3_FC_GABA_',date,'.pdf']);
end
% % % % % % % % % % % % % % 
[ind_gr_tmp_mark,cells_bor,gr_center] = markertablefeatures(T_cells_tmp(:,1),data_sorted_all,1);
list =  geneid(ind_gr_tmp_mark(1:end));
...
if savefig_flag==1
    savefig(gcf,['clustermarkers_final_step3_clusteraverage_FC_GABA_',date,'.fig'])
    eval(['export_fig clustermarkers_final_step3_clusteraverage_FC_GABA_',date,'.pdf']);
end

%%
% % % % % % % % % % % % % % % % % % % % % % % % % 
load clade_markers_list_GABA_hannah.mat
load clustercolor_GABA_hannah.mat%clustername_color_hannah
...
list = unique([list1;list2;list3;list4;list5;list6;list7;treemark1]);
logflag = 0;
for jjj=1:length(list)
    jjj
    hf1 = figure;
    set(gcf,'color','w','position',[20,20,1000,30],'Visible','off');
    gn = list{jjj}
    g = find(strcmpi(gn,geneid_all));
%     p99 = prctile(data_orig_all_sorted(g,:),100);
    p99 = max(p90_mat(g,:));
...
%             [f,xi] = ksdensity(y,'support',[0-0.1,0.1+max(y)]);
            ...
            plot(k + fi'.*(0.9*rand(length(gr2),1)-0.45), 0.5*rand(length(gr2),1)-0.1+y','.','color',['#',clustername_color_hannah{k,2}]); hold on;
        else
            plot(k+0.7*rand(length(gr2),1)-0.1,0.5*rand(length(gr2),1)-0.1+data_orig_all_sorted(g,gr2)','.','color',['#',clustername_color_hannah{k,2}]); hold on;
        end
        t_ed(k) = median(data_orig_all_sorted(g,gr2));
        t_av(k) = mean(data_orig_all_sorted(g,gr2));
        t_75(k) = prctile(data_orig_all_sorted(g,gr2),75);
    ...
    save2png(['GABA_singlegene_violin_220222/',gn,'_max',num2str(round(yt(end))),'_GABA_violinSingle_',date],gcf,1000)
    close(hf1)
end
% % % % % % % % % % % % % % 
%%
...
list = {'Ptk2b','Meis2','Wfs1','Maf','Zeb2','Satb1','Tshz1','Drd1','Pdyn','Drd2','Adora2a','Foxp2','Nfib','Lhx6','Resp18','Ppp1r1b'};
figure;
set(gcf,'color','w','position',[20,20,1100,960])
...
for i=1:length(list)
    ...
    axes(ha(i));
    scatter(mapped_xy(~inpos,1),mapped_xy(~inpos,2),10,markergene_color(~inpos,:),'.'); hold on;
    scatter(mapped_xy(inpos,1),mapped_xy(inpos,2),10,markergene_color(inpos,:),'.'); hold on;
...
if savefig_flag==1
    ...
    savefig(gcf,['tsne_step3_FC_GABA_hannah1_',date,'.fig'])
end
if savefig_pdf==1
    eval(['export_fig tsne_step3_FC_GABA_hannah1_',date,'.pdf']);
end
%%
load clade_markers_list_GABA_hannah.mat

list = unique([list1;list2;list3;list4;list5;list6;list7]);

list = [list;{'Ptk2b','Meis2','Wfs1','Maf','Zeb2','Satb1','Tshz1','Drd1'....
    ,'Pdyn','Drd2','Adora2a','Foxp2','Nfib','Lhx6','Resp18','Ppp1r1b','Penk','Gpr88'}'];
list = unique(list);
...
%     title(genePlot);
    ...
    set(gcf,'Renderer','OpenGL');
    set(gcf,'PaperPositionMode','auto');
    save2png(['tsne_singlegene_211201/',genePlot,'_GABA_tsneSingle_',date],gcf,1000)
    close(hf1)
end
