...
[prj,m,D,V,Q] = pca_wis(meangr_mat1',20);
% prj = prj_mean';
% prj = meangr_mat1';
...
mapped_xy_clusters = tsne(prj,'Algorithm','exact','Distance','correlation','NumDimensions',2,'NumPCAComponents',0,.....
    'Perplexity',perplexity,'Standardize',true,'InitialY',init,'LearnRate',500,'Verbose',1,'Options',options,'Exaggeration',exagg);
...
for i=1:length(T_cells_tmp_uni)
    if i>=0
        ...
        ht = text(mapped_xy_clusters(in,1)+xo,mapped_xy_clusters(in,2)+yo,regexprep(clusteruni{i},'_','-'));
        set(ht,'fontsize',8,'FontName','arial');%,'BackgroundColor',0.8*[1,1,1]
    end
end
% for i=1:length(T_cells_tmp_uni)
