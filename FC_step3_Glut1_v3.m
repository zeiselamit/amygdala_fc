...
cellidclsuter_gaba = [cellidclsuter_gaba, cellfun(@(x,y) ['Glut1_',num2str(x),'_',num2str(y)],cellidclsuter_gaba(:,2),cellidclsuter_gaba(:,3),'UniformOutput',0)]; 

gaba_clustername = loadCellFile('FC_Glut1_cluster_list_211007');
gaba_clustername(:,2) = regexprep(gaba_clustername(:,2),'_glut1','');
gaba_clustername(:,2) = cellfun(@(x) ['Glut1_',x], gaba_clustername(:,2),'UniformOutput',0);
gaba_clustername(:,2) = regexprep(gaba_clustername(:,2),'Glut1_exc','exc');
...
clustername_color_hannah = loadCellFile('Glut1-hexa.txt');
...
if savefig_flag==1
    savefig(gcf,['tsne_final_step3_FC_Glut1_by_cluster_',date,'.fig'])    
end
if savefig_pdf==1
    eval(['export_fig tsne_final_step3_FC_Glut1_by_cluster_',date,'.pdf']);
end
% 
...
linkaxes([h1,h2],'y');
if savefig_flag==1
    savefig(gcf,['tree_final_step3_FC_Glut1_',date,'.fig'])    
end
if savefig_pdf==1
    eval(['export_fig tree_final_step3_FC_Glut1_',date,'.pdf']);
end
...
saveCellFile(table1,['FC_final_step3_Glut1_dendrogram_junction_split_markers_',date,'.txt']);
saveCellFile(table2,['FC_final_step3_Glut1_dendrogram_junction_split_markers_by_average_',date,'.txt']);
...
% plot by tissue
if savefig_flag==1
    eval(['export_fig tsne_final_step3_FC_Glut1_by_annot_','_',date,'.pdf']);
    savefig(gcf,['tsne_final_step3_FC_Glut1_by_annot_perplexity_',date,'.fig'])
end
...
if savefig_flag==1
    eval(['export_fig tsne_final_step3_FC_Glut1_bytimepoint_',date,'.pdf']);
    savefig(gcf,['tsne_final_step3_FC_Glut1_bytimepoint_',date,'.fig'])
end
% % 
figure;
set(gcf,'color','w','position',[20,20,1100,960])
...
title('recall')
% if savefig_flag==1
%     eval(['export_fig tsne_final_FC_Glut1_bytimepoint_','_',date,'.pdf']);
%     savefig(gcf,['tsne_final_FC_Glut1_bytimepoint_',num2str(perplexity),'_PCAdim',num2str(initial_dims),'_',date,'.fig'])
% end
% % % % % % % % % % % % % % % % % % 
...
if savefig_flag==1
    savefig(gcf,['markertable_step3_final_FC_Glut1_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
if savefig_pdf==1
    eval(['export_fig markertable_step3_final_FC_Glut1_',date,'.pdf']);
end

table1 = [cellid_sorted,m2c(T_cells_tmp)];
saveCellFile(table1,['cellidCluster_step3_Glut1_FC_',date,'.txt']);
saveCellFile([m2c([1:length(leaforder_pca)]'),clusteruni(leaforder_pca)],['ClusterOrder_step3_Glut1_FC_',date,'.txt']);
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
linkaxes([ax1,ax2_1,ax2_2,ax2_3,ax2_4,ax2_5],'x');
if savefig_flag==1
    savefig(gcf,['junctionmarkers_final_step3_FC_Glut1_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
% % % % % % % % % 
...
if savefig_flag==1
    savefig(gcf,['junctionmarkers_final_step3_clusteraverage_FC_Glut1_',date,'.fig'])
    eval(['export_fig junctionmarkers_final_step3_clusteraverage_FC_Glut1_',date,'.pdf']);
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % % 
% % % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
linkaxes([ax1,h2],'x')
if savefig_flag==1
    savefig(gcf,['CEA_markers_step3_clusteraverage_FC_Glut1_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
linkaxes([ax1,h2],'x')
if savefig_flag==1
    savefig(gcf,['N_Z_list_step3_clusteraverage_FC_Glut1_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
...
if savefig_flag==1
    savefig(gcf,['clustermarkers_violin_final_step3_FC_Glut1_',date,'.fig'])
    eval(['export_fig clustermarkers_violin_final_step3_FC_Glut1_',date,'.pdf']);
end
% % % % % % % % % % % % % % 
[ind_gr_tmp_mark,cells_bor,gr_center] = markertablefeatures(T_cells_tmp(:,1),data_sorted_all,1);
list =  geneid(ind_gr_tmp_mark(1:end));
...
if savefig_flag==1
    savefig(gcf,['clustermarkers_final_step3_clusteraverage_FC_Glut1_',date,'.fig'])
    eval(['export_fig clustermarkers_final_step3_clusteraverage_FC_Glut1_',date,'.pdf']);
end
% % % % % % % % % % % % % % % % % % % % 
% gn = 'scrib';
% g = find(strcmpi(gn,geneid_all));
...
list = loadCellFile('Glut1-markers.txt');
list = list(2:end,:);
[ind_gr_tmp_mark,cells_bor,gr_center] = markertablefeatures(T_cells_tmp(:,1),data_sorted_all,1);
list2 =  geneid(ind_gr_tmp_mark(1:end));
list = unique([list;treemark1;list2]);
logflag = 0;
for jjj=1:length(list)
    jjj
    gn = list{jjj}
    g = find(strcmpi(gn,geneid_all));
    hf1 = figure;
    set(gcf,'color','w','position',[200,200,1000,30],'Visible','off');
...
%             [f,xi] = ksdensity(y,'support',[0-0.1,0.1+max(y)]);
    ...
    save2png(['glut1_singlegene_violin_220221/',gn,'_max',num2str(round(yt(end))),'_Glut1_violinSingle_',date],gcf,1000)
    close(hf1)
end
%%