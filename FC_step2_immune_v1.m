...
validcells = (gaba_glut==1 ) & amy_flag ;
sum(validcells)
...
saveCellFile(table1,['gene_correlation_immune_',date,'.txt'])
% % % % % % % % % % % 
% % % % % % % % % % % 
...
[prj,m,D,V,Q] = pca_wis(meangr_mat1',size(meangr_mat1,2));
...
C1qc = data_orig_all_sorted(strcmpi(geneid_all,'C1qc'),:);
Aif1 = data_orig_all_sorted(strcmpi(geneid_all,'Aif1'),:);
Cx3cr1 = data_orig_all_sorted(strcmpi(geneid_all,'Cx3cr1'),:);

ax6 = axes('position',[0.1,0.95,0.88,0.01]);
imagesc(~C1qc); hold on;
axes(ax6)
colormap('gray');
freezeColors(ax6);
set(ax6,'xtick',[],'ytick',[1],'yticklabel','C1qc','fontsize',5);
ax7 = axes('position',[0.1,0.95+0.01,0.88,0.01]);
imagesc(~Aif1); hold on;
colormap('gray');
freezeColors(ax7);
set(gca,'xtick',[],'ytick',[1],'yticklabel','Aif1','fontsize',6);
ax8 = axes('position',[0.1,0.95+2*0.01,0.88,0.01]);
imagesc(~Cx3cr1); hold on;
colormap('gray');
freezeColors(ax8);
set(gca,'xtick',[],'ytick',[1],'yticklabel','Cx3cr1','fontsize',6);
...
saveCellFile(table1,['cellidCluster_step2_immune_FC_',date,'.txt']);
