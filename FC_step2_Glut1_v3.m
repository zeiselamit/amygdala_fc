...
validcells = (gaba_glut==2 ) & amy_flag & (sum(data(loc,:)>1)')<2;
sum(validcells)
...
saveCellFile(table1,['gene_correlation_Glut1_',date,'.txt'])
% % % % % % % % % % % 
...
linkaxes([h1,h2],'y');
if savefig_flag==1
    savefig(gcf,['tree_FC_Glut1_',date,'.fig'])    
end
if savefig_pdf==1
    eval(['export_fig tree_FC_Glut1_',date,'.pdf']);
end
...
saveCellFile(table1,['FC_Glut1_dendrogram_junction_split_markers_',date,'.txt']);
saveCellFile(table2,['FC_Glut1_dendrogram_junction_split_markers_by_average_',date,'.txt']);
...
if savefig_flag==1
    savefig(gcf,['tsne_FC_Glut1_by_cluster_',date,'.fig'])
...
if savefig_pdf==1
    eval(['export_fig tsne_FC_Glut1_by_cluster_',date,'.pdf']);
end
...
if savefig_flag==1
    ...
    savefig(gcf,['tsne_FC_Glut1_by_annot_perplexity_',num2str(perplexity),'_PCAdim',num2str(initial_dims),'_',date,'.fig'])
end
...
if savefig_flag==1
    savefig(gcf,['markertable_FC_Glut1_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
if savefig_pdf==1
    eval(['export_fig markertable_FC_Glut1_',date,'.pdf']);
end

table1 = [cellid_sorted,m2c(T_cells_tmp)];
saveCellFile(table1,['cellidCluster_step2_glut1_FC_',date,'.txt']);
...
linkaxes([ax1,ax2_1,ax2_2,ax2_3,ax2_4,ax2_5,ax3,ax4,ax5,ax6,ax7,ax8],'x');
if savefig_flag==1
    savefig(gcf,['junctionmarkers_FC_Glut1_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
% % % % % % % % % 
...
if savefig_flag==1
    savefig(gcf,['junctionmarkers_clusteraverage_FC_Glut1_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
linkaxes([ax1,h2],'x')
if savefig_flag==1
    savefig(gcf,['CEA_markers_clusteraverage_FC_Glut1_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
linkaxes([ax1,h2],'x')
if savefig_flag==1
    savefig(gcf,['N_Z_list_clusteraverage_FC_Glut1_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % 
