...
marker_exclud_cell = {'Slc32a1','Gad1','Gad2','C1qc','C1qa','C1qb','Gja1','Cx3cr1','Acta2','Ly6c1','Mfge8','Plp1'....
...
validcells = (gaba_glut==3 ) & amy_flag & (sum(data(loc,:)>1)')<2;
sum(validcells)
...
saveCellFile(table1,['gene_correlation_Glut2_',date,'.txt'])
% % % % % % % % % % % 
...
linkaxes([h1,h2],'y');
if savefig_flag==1
    savefig(gcf,['tree_FC_Glut2_',date,'.fig'])    
end
if savefig_pdf==1
    eval(['export_fig tree_FC_Glut2_',date,'.pdf']);
end
...
saveCellFile(table1,['FC_Glut2_dendrogram_junction_split_markers_',date,'.txt']);
saveCellFile(table2,['FC_Glut2_dendrogram_junction_split_markers_by_average_',date,'.txt']);
...
if savefig_flag==1
    savefig(gcf,['tsne_FC_Glut2_by_cluster_',date,'.fig'])
...
if savefig_pdf==1
    eval(['export_fig tsne_FC_Glut2_by_cluster_',date,'.pdf']);
end
...
if savefig_flag==1
    ...
    savefig(gcf,['tsne_FC_Glut2_by_annot_perplexity_',num2str(perplexity),'_PCAdim',num2str(initial_dims),'_',date,'.fig'])
end
...
if savefig_flag==1
    savefig(gcf,['markertable_FC_Glut2_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
if savefig_pdf==1
    eval(['export_fig markertable_FC_Glut2_',date,'.pdf']);
end

table1 = [cellid_sorted,m2c(T_cells_tmp)];
saveCellFile(table1,['cellidCluster_step2_Glut2_FC_',date,'.txt']);
...
linkaxes([ax1,ax2_1,ax2_2,ax2_3,ax2_4,ax2_5,ax3,ax4,ax5,ax6,ax7,ax8],'x');
if savefig_flag==1
    savefig(gcf,['junctionmarkers_FC_Glut2_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
% % % % % % % % % 
...
if savefig_flag==1
    savefig(gcf,['junctionmarkers_clusteraverage_FC_Glut2_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
linkaxes([ax1,h2],'x')
if savefig_flag==1
    savefig(gcf,['CEA_markers_clusteraverage_FC_Glut2_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
linkaxes([ax1,h2],'x')
if savefig_flag==1
    savefig(gcf,['N_Z_list_clusteraverage_FC_Glut2_',date,'.fig'])
    % eval(['export_fig markertable_AmyPiri_FC_',date,'.pdf']);
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % 
