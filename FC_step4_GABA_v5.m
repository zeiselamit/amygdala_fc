...
for i=1:length(clusteruni)
    ind = find(strcmpi(cellid_clusters(:,4),clusteruni{i,1}));
    T_cells_tmp(ind) = i;
...
%             [f,xi] = ksdensity(y,'support',[0-0.1,0.1+max(y)]);
    ...
    title(regexprep(clusteruni{c,2},'_','-'));%[gn,',c=',num2str(c)])
end
linkaxes(ha,'y');
if 1==100
...
gr1 = find((T_cells_tmp==51 & fc_time==0 & batch_flag>=2));
gr2 = find((T_cells_tmp==51 & fc_time==8 & batch_flag>=2));
...
gr1 = find((T_cells_tmp==9 & fc_time==0 & batch_flag>=2));
gr2 = find((T_cells_tmp==9 & fc_time==8 & batch_flag>=2));
...
%     list = {'Arc','Fos','Fosb','Fosl2','Npas4','Btg2','Jun','Junb','Egr1','Egr2','Egr4','Nr4a1','Nr4a3','Dusp1'};
    list = {'Jun','Egr1','Egr2','Egr4'...
    ,'Dusp1','Dusp5','Dusp6','Btg1','Nr4a1','Homer1','Ptgs2','Atf3','Cdkn1a','Gadd45b'};
    hf = figure('color','w','position',[20,20,1200,200],'Name',clusteruni{c,2});
...
%             text([1,2,3,4,5],7.5*[1,1,1,1,1],cellfun(@num2str, m2c(grsz_batch23(c,:)),'UniformOutput',0),'fontsize',6)
            set(gca,'xtick',[1:5],'XTickLabel',[{'0','2','8','24','recall'}],'xlim',[1,5],'ylim',[0,9])
            
            %[gn,',c=',num2str(c)])
            if jjj==1
                ylabel(clusteruni{c,2},'Interpreter','none','fontsize',7)
                title(gn);
%                 title([clusteruni{c},',',gn],'Interpreter','none');
    ...
    linkaxes(ha,'xy');
    eval(['export_fig GABA_IEG_FC_220510/',clusteruni{c,2},'_IEG_FC',date,'.pdf']);
    close(hf)
end
%%
list = {'Arc','Fos','Fosb','Fosl2','Npas4','Btg2','Jun','Junb','Egr1','Egr2','Egr4','Nr4a1','Nr4a3','Dusp1'};
...
%%
% ieggenes = {'Fos','Fosl2','Fosb','Jun','Junb','Egr1','Egr2','Egr4'...
...
% text([1:length(s)]-4,s,cellfun(@num2str , m2c(sgrsz),'UniformOutput',0),'Interpreter','none','fontsize',6,'color','m')


% scatter(find(v==1),s(v),sgrsz(v)*0.3,repmat([1,0,0],sum(v),1),'filled');hold on;
% scatter(find(v==0 & sgrsz>60),s(v==0 & sgrsz>60),sgrsz(v==0 & sgrsz>60)*0.3,repmat([1,1,1]*.5,sum(v==0),1),'filled');
% scatter(find(v==0 & sgrsz>60 & min_sgrsz>10),s(v==0 & sgrsz>60 & min_sgrsz>10),sgrsz(v==0 & sgrsz>60)*0.3,repmat([1,1,1]*.5,sum(v==0),1),'filled');
% scatter(find(v==0),s(v==0),sgrsz(v==0)*0.3,repmat([1,1,1]*.5,sum(v==0),1),'filled');
% text([1:length(s)]+2,s,clusteruni(xi,2),'Interpreter','none','fontsize',4)
% text([1:length(s)]-4,s,cellfun(@num2str , m2c(sgrsz),'UniformOutput',0),'Interpreter','none','fontsize',4,'color','m')

axis tight
ylabel('IEG score');
xlabel('rank');
set(gca,'xlim',[-5,length(xi)+8])
eval(['export_fig GABA_IEGfrac_score_scatter_p90_grsz20_',date,'.pdf']);

table1 = [clusteruni(xi,2),m2c([s,sgrsz,min_sgrsz])];
saveCellFile(table1,'GABA_IEGfrac_score_p90_grsz20.txt')
...
%     ,regexprep(clusteruni(xi,2),'_','-'),'fontsize',7);
% set(gca,'ytick',[1:length(c)],'yticklabel',repmat([ieggenes,{'any'}]',length(T_cells_tmp_uni),1));   
% set(gca,'xtick',[1:5],'xticklabel',{'0','2','8','24','28'})

gaba_cluster_by_iegscore = clusteruni(xi(v));
...
legend(ieggenes)
eval(['export_fig top5_clusters_GABA_IEGfrac_p90_grsz20_',date,'.pdf']);
...
legend(ieggenes)
eval(['export_fig top5_clusters_GABA_IEG_fold_frac_p90_grsz20_',date,'.pdf']);
...
legend(ieggenes)
eval(['export_fig low5_clusters_GABA_IEGfrac_p90_grsz20_',date,'.pdf']);
...
legend(ieggenes)
eval(['export_fig low5_clusters_GABA_IEG_fold_frac_p90_grsz20_',date,'.pdf']);
...
%     end
%     set(ha(i),'xtick',[0,2,8,24,26])
%     title(regexprep(clusteruni{xi(i),2},'_','-'))
%     axis tight
...
eval(['export_fig IEG_pergene_heatmap_GABA_p90_grsz20_',date,'.pdf']);
figure;
imagesc(tp_ptp_gene);
set(gca,'ytick',[1:length(ieggenes)],'yticklabel',ieggenes,'xtick',[1:5],'xticklabel',{'0','2','8','24','26'})
...
eval(['export_fig ratio_IEG_pertimepoint_GABA_p90_3IEG_grsz20_top5_',date,'.pdf']);
%% IEG +/- scatters
ieggenes = {'Bdnf','Homer1','Fos','Arc','Btg2','Fosl2','Npas4','Nr4a1'};
[~,loc] = ismember(ieggenes,geneid);
pcth = 95;
...
[~,gg] = ismember(gaba_cluster_by_iegscore,clusteruni(v,1));
v = v(gg(gg>0));
...
saveCellFile(list,['GABA_up_d05_1_IEGposneg_',date,'.txt']);
% 
...
saveCellFile(list,['GABA_down_d05_1_IEGposneg_',date,'.txt']);
eval(['export_fig GABA_updown_d05_1_IEGposneg_',date,'.pdf']);
% % 
% ax2 = axes('Position',[0.6,0.1,0.4,0.87]);
...
in = find(mean(data>0,2)>0.05 & mean(data>0,2)<0.5);
list = loadCellFile('GABA_topgenes_d_IEGposneg_02-May-2022.txt');
[~,in] = ismember(list,geneid);
pcth = 90;
d = data(in,:);
dthresh = prctile(d,pcth,2);
in = in(dthresh>=0);
d = data(in,:);
dthresh = prctile(d,pcth,2);
d95 = d>repmat(dthresh,1,length(d));
genefrac_mat = cell(length(T_cells_tmp_uni),1);
genefrac_mat_fold = cell(length(T_cells_tmp_uni),1);
for k=1:length(T_cells_tmp_uni)
    k
    if min(grsz_batch23(k,:),[],2)>20
        tmp =zeros(length(in),5);
        for jj=1:5
            gr1 = find((T_cells_tmp==k & fc_time==fc_time_uni(jj) & batch_flag>=2));
            tmp(:,jj) = [mean(d95(:,gr1),2)];
        end
        genefrac_mat{k} = tmp;
        tmp1 = tmp(:,1);
        tmp1(tmp1==0) = 1/grsz_batch23(k,1);
        genefrac_mat_fold{k} = tmp./repmat(tmp1,1,length(fc_time_uni));
    end
end


c = cell2mat(genefrac_mat');
c(c==0) = 1;
figure; 
plot(log2(mean(c,2)),log2(std(c,[],2)./mean(c,2)),'.');
% figure; 
% plot(,log2(std(c,[],2)./mean(c,2)),'.');
[~,xi] = sort(sum(c>2,2),'descend');

cv_c = std(c,[],2)./mean(c,2);
[~,xi] = sort(cv_c,'descend');
perplexity = 30;
options = statset('MaxIter',1000);
mapped_xy = tsne(c,'Algorithm','barneshut','Distance','correlation','NumDimensions',2,'NumPCAComponents',0,.....
    'Perplexity',perplexity,'Standardize',true,'InitialY',[],'LearnRate',500,'Theta',0.5,'Verbose',1,'Options',options,'Exaggeration',1);
figure; plot(mapped_xy(:,1),mapped_xy(:,2),'.'); hold on;
text(mapped_xy(:,1),mapped_xy(:,2),geneid(in),'fontsize',8)
...
for k=1:length(T_cells_tmp_uni)
    ...
    if min(grsz_batch23(k,:),[],2)>20
        ...
        iegpos = iegpos(gr1);
        d = data(:,gr1);
        in = find(mean(d>0,2)>0.05 & mean(d>0,2)<0.99);
        d = d(in,:);
...
gr1name = 'gr1-IEG(-)';
gr2name = 'gr2-IEG(+)';
for k=18:length(T_cells_tmp_uni)
    k
    %     if min(grsz_batch23(k,1))>30
    gr1 = find((T_cells_tmp==k & fc_time>=2 & batch_flag>=2));
    if gr1>100
    ...
    if min(grsz_batch23(k,:),[],2)>20
        ...
        [ha, pos] = tight_subplot(5, 3, [0.05,0.05], [0.1,0.05], [0.05,0.05]);
        s = 1;
        for jj=1:5
            
            gr1 = find((T_cells_tmp==k & fc_time==fc_time_uni(jj) & batch_flag>=2 & ~iegpos'));
            gr2 = find((T_cells_tmp==k & fc_time==fc_time_uni(jj) & batch_flag>=2 & iegpos'));
            ...
            %         eval(['export_fig GABA_scatter_IEG_pos_neg_FC_220423/',clusteruni{k},'_scatter_IEG_FC',date,'.pdf']);
        end
        pause
        close(hf)
    end
...
for k=1:length(T_cells_tmp_uni)
    ...
    if min([length(gr1),length(gr2)])>10
        ...
        ylabel(['2hr-recall'])
        axis tight
        eval(['export_fig GABA_scatter_2hr_recall_90p_220525/',clusteruni{k,2},'_scatter_2hr_recall_90p_',date,'.pdf']);
        %     pause
        close(hf)
    end
end
...
ylabel('genes')
xlabel('#celltypes d<-1')
eval(['export_fig GABA_topgenes_2hr_recall_90p_',date,'.pdf']);
table1 = [ {'upreg','downreg'};[topup,todown] ];
saveCellFile(table1,['topgenes_2hr_recall_90p_GABA_',date,'.txt']);
...
for k=1:length(T_cells_tmp_uni)
    ...
    if min([length(gr1),length(gr2)])>10
        ...
        ylabel(['2hr-recall'])
        axis tight
        eval(['export_fig GABA_scatter_8hr_24hr_90p_220602/',clusteruni{k,2},'_scatter_8hr_24hr_90p_',date,'.pdf']);
        %     pause
        close(hf)
    end
end
...
ylabel('genes')
xlabel('#celltypes d<-1')
eval(['export_fig GABA_topgenes_8hr_24hr_90p_',date,'.pdf']);
table1 = [ {'upreg','downreg'};[topup,todown] ];
saveCellFile(table1,['topgenes_8hr_24hr_90p_GABA_',date,'.txt']);


%% gene correlation per cluster
list1 = {'Egr1','Egr4','Junb','Scg2','Ntrk2','Vgf','Dclk1','Syt4','Clstn3','Pde10a','Ptprn','Zdbf2','Syt4','Nptxr'....
,'Arpp21','Sv2c','Tiparp','Wipf3','Ptpn5','Plppr4','Sorcs3','Chgb','Nrxn3','Nptx2','Kcnv1','Kcnf1','Lgi2'};
...
[ha, pos] = tight_subplot(1, 5, [0.02,0.02], [0.05,0.1], [0.05,0.02]);
for i=1:5
    k = find(strcmpi(gaba_cluster_by_iegscore{i},clusteruni));
    axes(ha(i))
...
[ha, pos] = tight_subplot(1, 5, [0.02,0.02], [0.05,0.1], [0.05,0.02]);
for i=1:5
    k = find(strcmpi(gaba_cluster_by_iegscore{end-i},clusteruni));
    axes(ha(i))
    imagesc(iegfram_mat_fold{k},[0,3])
    if i==1
        set(gca,'xtick',[1:5],'xticklabel',[0,2,8,24,26],'ytick',[1:length(ieggenes)],'yticklabel',ieggenes);
...
for k=1:length(T_cells_tmp_uni)
    ...
    for jj=1:5
        gr1 = find(T_cells_tmp == T_cells_tmp_uni(k) & batch_flag>=2 & fc_time==fc_time_uni(jj));
        r = corr_mat(d(:,gr1)');
        r(isnan(r)) = 0;
        t = r.*sqrt((length(gr1)-2)./(1-r.^2));
        p = 2*(1-tcdf(abs(t),length(gr1)-2));
...
%     pause(5)
    eval(['export_fig GABA_IEGmemgenes_correlation_220606/',clusteruni{k,2},'_corrP_IEGmemG_',date,'.pdf']);
    close(hf)
...
%%
ieggenes = [{'Bdnf','Npas4','Creb1' ,'Creb3l1' ,'Creb3l4' ,'Creb3' ,'Creb3l2' ,'Creb5' ...
    ,'Crebl2' ,'Crebzf' ,'Creb3l3' ,'Crebbp' ,'Crebrf' ,'Nrxn3' ,'Nrxn1' ,'Nrxn2' ,'Nlgn3' ,'Nlgn1' ...
    ,'Nlgn2' ,'Nxph2' ,'Nxpe5' ,'Nxph1' ,'Nxph4' ,'Nxpe2' ,'Nxpe4' ,'Nxph3' ,'Nxpe3'}];
[~,loc] = ismember(upper(ieggenes),upper(geneid));
ieggenes(loc==0) = [];
...
i_nr4a1 = find(strcmpi('Nr4a1',ieggenes));
...
for k=1:length(T_cells_tmp_uni)
    ...
    if min(grsz_batch23(k,:))>20
        ...
        for jj=1:6
            ...
            [~,prbdnf] = corr(d(i_nr4a1,gr1)',data(inexp,gr1)');
            ...
            bdnfcorr(:,jj) = p(:,i_nr4a1)<0.01;
            nlgn2corr(:,jj) = p(i_nlgn1,:)<0.01;

            axes(ha(jj))
            %     hf = figure('position',[100,100,800,800],'color','w');
            imagesc(-log10(p(sorted_ind,sorted_ind)+1e-5),[0,3]);title(clusteruni{k,2},'Interpreter','none')
            set(gca,'xtick',[1:length(ieggenes)],'xticklabel',ieggenes(sorted_ind),'ytick',[1:length(ieggenes)],'yticklabel',ieggenes(sorted_ind))
            colormap("summer")
%             colormap(cm)
        ...
        colorbar('position',[0.97,0.02,0.01,0.9])
        linkaxes(ha,'xy')
%         eval(['export_fig GABA_IEGmemgenes_correlation_220712/',clusteruni{k,2},'_corrP_IEGmemG_p3_',date,'.pdf']);
...
%         set(gca,'ytick',[1:length(intmp)],'yticklabel',ieggenes(xi(intmp)),'xtick',[1:6],'xticklabel',{'0','0','2','8','24','recall'})
%         title('Nlgn2')
%         eval(['export_fig GABA_IEGmemgenes_correlation_220704/',clusteruni{k,2},'_Bdnf_Nlgn2_IEGmemG_',date,'.pdf']);
        pause
        %     eval(['export_fig GABA_IEGmemgenes_correlation_220606/',clusteruni{k,2},'_corrP_IEGmemG_',date,'.pdf']);
        close(hf)
%         close(hf2)
...
saveCellFile(table1,['corrscore_GABA_',date,'.txt'])
...
set(gca,'ytick',[1:length(T_cells_tmp_uni(validct_in_sort))],'yticklabel',cell(sum(validct_in_sort),1),'ticklabelinterpreter','none',...
    'xtick',[1:6],'xticklabel',{'0','0','2','8','24','recall'});
title('SEMA ');
% eval(['export_fig Corrscore_GABA_',date,'.pdf']);
...
intmp = find(sum(bdnf_corrgenes_bin(:,[3:6:end,4:6:end,5:6:end,6:6:end]),2)>=10);
validct_in = find(validct);
...
box off
% eval(['export_fig Nr4a1_10_Corr_GABA_',date,'.pdf']);
...
saveCellFile([geneid(intmp(xi)),m2c(z)],['genes_corr_with_Nr4a1_GABA_',date,'.txt'])

ngln2_corrgenes_bin = nlgn2_corrgenes<1e-1;
intmp = find(sum(ngln2_corrgenes_bin(:,[3:6:end,4:6:end,5:6:end,6:6:end]),2)>=7);
validct_in = find(validct);
intime = [(validct_in(xiscoreall)-1)*6+repmat([1:6],sum(validct),1)];
intime = intime(:);
...
box off
set(gca,'xtick',[1:length(intime)],'xticklabel',cell(length(intime),1))
% eval(['export_fig Nlgn1_7_Corr_GABA_',date,'.pdf']);
...
saveCellFile([geneid(intmp(xi)),m2c(z)],['genes_corr_with_Nlgn1_GABA_',date,'.txt'])
%%
ieg = [{'Bdnf','Homer1','Fos','Arc','Btg2','Fosl2','Npas4','Nr4a1'},{'Fosb','Junb','Egr1','Egr2','Egr4','Nr4a3','Dusp1','Dusp6','Cdkn1a','Gadd45b','Inhba'}];
ieggenes = [ieg];
[~,loc] = ismember(upper(ieggenes),upper(geneid));
...
for k=1:length(T_cells_tmp_uni)
    ...
    if min(grsz_batch23(k,:))>20
        ...
        colorbar('position',[0.97,0.02,0.01,0.9])
        linkaxes(ha,'xy')
        eval(['export_fig GABA_IEG_correlation_220712/',clusteruni{k,2},'_corrP_IEG_',date,'.pdf']);        
        close(hf)
    end
end
