...
cellidclsuter_gaba = loadCellFile('cellidCluster_step2_Glut2_FC_07-Oct-2021.txt');
cellidclsuter_gaba = [cellidclsuter_gaba, cellfun(@(x,y) ['Glut2_',num2str(x),'_',num2str(y)],cellidclsuter_gaba(:,2),cellidclsuter_gaba(:,3),'UniformOutput',0)]; 

gaba_clustername = loadCellFile('FC_Glut2_cluster_list_211007');
gaba_clustername(:,2) = regexprep(gaba_clustername(:,2),'_glut2','');
gaba_clustername(:,2) = cellfun(@(x) ['Glut2_',x], gaba_clustername(:,2),'UniformOutput',0);
gaba_clustername(:,2) = regexprep(gaba_clustername(:,2),'Glut2_exc','exc');
...
list = unique([list2;list;treemark1]);
logflag = 0;
for jjj=1:length(list)
    jjj
    gn = list{jjj}
...
%             [f,xi] = ksdensity(y,'support',[0-0.1,0.1+max(y)]);
    ...
    save2png(['glut2_singlegene_violin_220221/',gn,'_max',num2str(round(yt(end))),'_Glut2_violinSingle_',date],gcf,1000)
    close(hf1)
end
%%